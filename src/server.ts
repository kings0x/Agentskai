...
function attachTerminal(socket: { send: (data: string) => void; on: (event: string, handler: (...args: any[]) => void) => void; close: (code?: number, reason?: string) => void }, session: Session): void {
  ...
  socket.on('message', (raw: Buffer | string) => { try { const message = JSON.parse(raw.toString()) as { type?: string; data?: string; cols?: number; rows?: number; lines?: number; requestId?: string }; if (message.type === 'input' && typeof message.data === 'string') { if (message.data.length > 65_536) throw new Error(); session.write(message.data, message.requestId); if (message.requestId) socket.send(JSON.stringify({ type: 'ack', requestId: message.requestId })); } else if (message.type === 'resize' && message.cols && message.rows) session.resize(message.cols, message.rows); else if (message.type === 'scroll' && Number.isInteger(message.lines) && Math.abs(message.lines!) <= 100 && message.lines !== 0) session.scroll(message.lines!); else throw new Error(); } catch { socket.send(JSON.stringify({ type: 'error', message: 'Invalid terminal message' })); } });
